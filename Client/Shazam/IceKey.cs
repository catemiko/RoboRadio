    ...
    public class IceKey
    {
        ...
        private long perm32(long x)
        {
            ...
            while (x != 0L)
            {
                if ((x & 1L) != 0L)
                {
                    num |= (long)((ulong)IceKey.pBox[(int)((IntPtr)num2)]);
                }
                num2 += 1L;
                x >>= 1;
            }
        ...
        public void set(char[] key)
        {
            ...
            for (int j = 0; j < this.size; j++)
            {
                for (int k = 0; k < 4; k++)
                {
                    array[3 - k] = (int)((int)key[j * 8 + k * 2] << 8 | key[j * 8 + k * 2 + 1]);
                }
                this.scheduleBuild(array, j * 8, 0);
                this.scheduleBuild(array, this.rounds - 8 - j * 8, 8);
            }
        }
        ...
        private ulong roundFunc(ulong p, int i, int[,] subkey)
        {
            ...
            return checked(IceKey.spBox[(int)((IntPtr)0L), (int)((IntPtr)((long)(num3 >> 10)))] | IceKey.spBox[(int)((IntPtr)1L), (int)((IntPtr)((long)(num3 & 1023uL)))] | IceKey.spBox[(int)((IntPtr)2L), (int)((IntPtr)((long)(num4 >> 10)))] | IceKey.spBox[(int)((IntPtr)3L), (int)((IntPtr)((long)(num4 & 1023uL)))]);
        }
        ...
        private void encrypt(byte[] plaintext, byte[] ciphertext, int idx)
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                ciphertext[idx + 3 - j] = (byte)(num2 & 255uL);
                ciphertext[idx + 7 - j] = (byte)(num & 255uL);
                num2 >>= 8;
                num >>= 8;
            }
        }
        ...
        private void decrypt(byte[] ciphertext, byte[] plaintext)
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                plaintext[3 - j] = (byte)(num2 & 255uL);
                plaintext[7 - j] = (byte)(num & 255uL);
                num2 >>= 8;
                num >>= 8;
            }
        }
        ...
        public string decString(string str)
        {
            ...
            for (int i = 0; i < num; i += 16)
            {
                ...
                for (int j = 0; j < 8; j++)
                {
                    array2[j] = Convert.ToByte(Convert.ToString(array[i + j * 2] + array[i + j * 2 + 1]), 16);
